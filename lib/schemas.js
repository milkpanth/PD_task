...
export const SCHEMAS = {
  ...
  timesheet: {
    // Timesheet has its own calendar view + modal (see app/pd/timesheet and
    // components/TimesheetModal.js) instead of GenericTable/GenericModal,
    // because entries branch into "เข้างาน" vs "ลา" with different fields.
    // This entry is kept so LEAVE_TYPES etc. have one canonical home and so
    // any future generic tooling (exports, search) still knows the table shape.
    label: 'Timesheet', icon: '🕒', table: 'timesheets', global: true,
    fields: [
      { key: 'date', type: 'date', label: 'วันที่ (Date)', required: true },
      { key: 'entryType', type: 'select', label: 'ประเภท', options: ['attendance', 'leave'] },
      { key: 'timeIn', type: 'time', label: 'เวลาเข้างาน (Time In)' },
      { key: 'timeOut', type: 'time', label: 'เวลาออกงาน (Time Out)' },
      { key: 'projectId', type: 'projectSelect', label: 'Project' },
      { key: 'workDetail', type: 'textarea', label: 'รายละเอียดงานที่ทำ' },
      { key: 'leaveType', type: 'select', label: 'ประเภทการลา', options: ['Annual Leave', 'Sick Leave', 'Personal Leave'] },
      { key: 'leaveReason', type: 'textarea', label: 'เหตุผลในการลา' },
    ],
  },
};

export const LEAVE_TYPES = ['Annual Leave', 'Sick Leave', 'Personal Leave'];
export const LEAVE_TYPE_LABEL_TH = {
...
export function genericStatusColor(v) {
  ...
  if (['in progress', 'retest', 'in sprint'].includes(s)) return 'status-inprogress';
  if (['review', 'analyst', 'gathering', 'next phase', 'in review', 'reviewing'].includes(s)) return 'status-review';
  return 'status-todo';
}
